<?php
  ...
  try {
      ...
      //Get average rating
      $averageRatingQuery = 'SELECT AVG(f.sellerRating) as average, u.username,  COUNT(f.sellerRating) as count 
                            FROM feedback AS f
                            JOIN `user` AS u ON u.userID = f.buyerID  
      ...
      $data['average'] = $buyersAverage  $getBuyersAverage->fetch(PDO::FETCH_OBJ);
